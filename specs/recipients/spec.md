...
- [ ] Recipients CRUD with address uniqueness (lowercase, DB index)
- [ ] Recipients CRUD with nickname uniqueness (trimmed, case-insensitive, DB index on normalized)
- [ ] Duplicate address shows existing recipient name and offers edit
- [ ] Duplicate nickname shows which address uses the name and blocks save
- [ ] Address format + EIP-55 checksum validation inline
- [ ] Treasury-owned address blocked on create and send, with pointer to internal transfer
- [ ] Wallet activity check: warning when no prior Tempo activity (non-blocking)
- [ ] Activity check unavailable: neutral status shown (non-blocking)
- [ ] Recipients page sorted by turnover with nickname, address, turnover, last active
- [ ] Recipients page empty state with "Add your first recipient" CTA
- [ ] ⋯ menu on each recipient: Edit, Delete, View Transactions
- [ ] View Transactions opens `/transactions` filtered to recipient address
- [ ] Recipient picker in send form (📇 icon), sorted by turnover, searchable by name/address
- [ ] Selecting recipient fills address + shows nickname above field
- [ ] Editing address after selection clears the nickname/selection state
- [ ] Ad-hoc save prompt after sending to a new (non-recipient, non-treasury) address
- [ ] Transaction list shows recipient nickname next to address for matching counterparties
- [ ] Transaction detail page shows recipient name when counterparty is saved
- [ ] Internal transfers and swaps do not show recipient names (they use account names)
- [ ] Incoming payment WebSocket toast shows "Payment received from [nickname]!" for saved recipients
- [ ] Stats cached in `recipient_stats` table, refreshed after transaction sync
- [ ] Stats backfill triggers immediately on recipient create, shows loading state until complete
- [ ] Deleting recipient cascades stats row
- [ ] "Recipients" appears in sidebar between "Accounts" and "Settings"
