...
contract GuardianFactory {
    ...
    function createGuardian(address agent, uint256 maxPerTx, uint256 dailyLimit, uint256 spendingCap, bytes32 salt)
        external
        returns (address guardian)
    {
        bytes32 effectiveSalt = _effectiveSalt(msg.sender, salt);
        guardian = address(new SimpleGuardian{salt: effectiveSalt}(msg.sender, agent, maxPerTx, dailyLimit, spendingCap));
    ...
    function getGuardianAddress(
        ...
        bytes32 initCodeHash = keccak256(
            abi.encodePacked(type(SimpleGuardian).creationCode, abi.encode(deployer, agent, maxPerTx, dailyLimit, spendingCap))
        );
        return address(
