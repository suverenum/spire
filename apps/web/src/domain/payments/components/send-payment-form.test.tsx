...
const validRecipient = "0xabcdef1234567890abcdef1234567890abcdef12" as `0x${string}`;
...
describe("SendPaymentForm", () => {
	...
	it("renders form when open", () => {
		renderWithQuery(<SendPaymentForm open={true} onClose={() => {}} fromAddress={addr} />);
	...
	it("shows validation errors on submit with empty form", () => {
		...
		expect(screen.getByText("Invalid address format (0x...)")).toBeInTheDocument();
	...
	describe("validation", () => {
		...
		it("shows error for memo exceeding 32 bytes", () => {
			...
			fireEvent.click(screen.getByRole("button", { name: /Send Payment/ }));
			expect(screen.getByText("Memo must be 32 bytes or less")).toBeInTheDocument();
		});
		...
		it("does not show memo error for exactly 32 bytes", () => {
			...
			fireEvent.click(screen.getByRole("button", { name: /Send Payment/ }));
			expect(screen.queryByText("Memo must be 32 bytes or less")).not.toBeInTheDocument();
		});
		...
		it("accepts valid address format", () => {
			...
			fireEvent.change(amountInput, { target: { value: "10" } });
			fireEvent.click(screen.getByRole("button", { name: /Send Payment/ }));
			expect(screen.queryByText("Invalid address format (0x...)")).not.toBeInTheDocument();
		});
	});
	...
	describe("form submission", () => {
		...
		it("resets form and calls onClose on successful mutation", () => {
			...
			renderWithQuery(<SendPaymentForm open={true} onClose={onClose} fromAddress={addr} />);
			const toInput = screen.getByLabelText("Recipient Address");
			const amountInput = screen.getByLabelText("Amount");
