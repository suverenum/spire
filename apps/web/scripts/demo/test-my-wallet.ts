...
function fmt(r) { return (Number(r) / 1e6).toFixed(2); }
...
const pub = createPublicClient({ chain: tempoModerato, transport: viemHttp("https://rpc.moderato.tempo.xyz") });
const agentW = createWalletClient({ account: agentAccount, chain: tempoModerato, transport: viemHttp("https://rpc.moderato.tempo.xyz") });
...
const mppx = Mppx_client.create({
    ...
    async onChallenge(challenge) {
        ...
        console.log(`${DIM}    402 → pay $${fmt(BigInt(amount))} to ${recipient.slice(0,10)}...${RESET}`);
...
const bal = await readContract(pub, { address: ALPHAUSD, abi: Tip20, functionName: "balanceOf", args: [GUARDIAN] });
